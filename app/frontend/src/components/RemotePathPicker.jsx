...
export default function RemotePathPicker({ value, onChange, onBrowse, placeholder, remoteUrl, disabled, suggestedSuffix }) {
  ...
  const hasSuffix = suggestedSuffix && value && value.endsWith(suggestedSuffix);
  const basePart = hasSuffix ? value.slice(0, -suggestedSuffix.length) : value;
    ...
    <div className="path-picker">
      <div className="path-input-group">
        <div className="path-input-wrapper">
          ...
          />
          {hasSuffix && (
            <div className="path-input-highlight" aria-hidden="true">
              <span className="highlight-base">{basePart}</span>
              <span className="highlight-suggested">{suggestedSuffix}</span>
            </div>
          )}
        </div>
        <button
        ...
        <div className="modal-overlay" onClick={() => setBrowsing(false)}>
          <div className="modal picker-modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <button type="button" className="btn btn-ghost btn-sm" onClick={() => setBrowsing(false)}>✕</button>
            </div>
