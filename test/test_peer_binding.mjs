...
db.exec(`
  ...
  INSERT INTO pairing_requests VALUES
    (1, 'outgoing', 'accepted', 'Kasbah', 'http://192.168.70.2:8091', 'kasbah-identity', 'enc:old-key'),
    (2, 'outgoing', 'accepted', 'Kasbah', 'http://192.168.70.9:8091', 'kasbah-identity', 'enc:new-key'),
    (3, 'outgoing', 'accepted', 'Rebuilt', 'http://192.168.70.2:8091', 'other-identity', 'enc:other-key'),
    (4, 'outgoing', 'pending',  'Later',   'http://192.168.70.3:8091', 'later-identity', 'enc:pending'),
    (5, 'incoming', 'accepted', 'Inbound', 'http://192.168.70.4:8091', 'inbound-identity', 'enc:inbound');
`);
...
assert.equal(findPairingByStaticKey(db, 'kasbah-identity').id, 2);
assert.equal(findPairingByUrl(db, 'http://192.168.70.2:8091').id, 3);

// Re-pairing rotates the key and may move the peer: the job follows both
const rePaired = resolvePeerBinding(db, {
  id: 1, remote_url: 'http://192.168.70.2:8091',
  peer_static_pubkey: 'kasbah-identity', remote_api_key_encrypted: 'enc:stale-job-key',
});
assert.equal(rePaired.source, 'identity');
assert.equal(rePaired.apiKeyEncrypted, 'enc:new-key');
assert.equal(rePaired.remoteUrl, 'http://192.168.70.9:8091');
assert.equal(rePaired.peerName, 'Kasbah');
...
const strangerAtSameUrl = resolvePeerBinding(db, {
  id: 2, remote_url: 'http://192.168.70.2:8091',
...
assert.equal(strangerAtSameUrl.remoteUrl, 'http://192.168.70.2:8091');

// Legacy jobs without an identity still resolve by address
const legacy = resolvePeerBinding(db, {
  id: 3, remote_url: 'http://192.168.70.2:8091',
...
db.prepare('INSERT INTO hyper_backup_jobs VALUES (1, ?, ?, ?, NULL)')
  .run('http://192.168.70.2:8091', 'kasbah-identity', 'enc:stale-job-key');
const job = db.prepare('SELECT * FROM hyper_backup_jobs WHERE id = 1').get();
assert.equal(syncJobRemoteUrl(db, job, 'http://192.168.70.9:8091'), true);
assert.equal(job.remote_url, 'http://192.168.70.9:8091');
assert.equal(db.prepare('SELECT remote_url FROM hyper_backup_jobs WHERE id = 1').get().remote_url,
  'http://192.168.70.9:8091');
assert.equal(syncJobRemoteUrl(db, job, 'http://192.168.70.9:8091'), false);
