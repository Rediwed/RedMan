...
assert.doesNotMatch(deploy, /\[\[ \"\\\$d_state\" -eq 0 \]\]/);
assert.match(deploy, /unrelated host task\(s\) in D-state/);
assert.match(deploy, /run_with_deadline 35 ssh/);
assert.match(deploy, /\.redman-deploy-io\.XXXXXX/);
assert.match(deploy, /mktemp/);
assert.match(deploy, /conv=fsync/);
assert.match(deploy, /trap cleanup_probe EXIT HUP INT TERM/);
assert.match(deploy, /DockerRootDir/);
assert.match(deploy, /readlink -f/);
assert.doesNotMatch(deploy, /docker_probe=\/var\/lib\/docker/);
