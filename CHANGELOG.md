...
### Fixed
- [x] Fixed backups failing indefinitely against a backup root that still holds content from before the restricted account existed. Such files are owned by root, and rsync mirrors the source's permissions — but changing a file's mode is refused to anyone except its owner, so every run ended with hundreds of *Operation not permitted* errors on files it could otherwise read and write. Nothing in the error pointed at ownership. The setup script now reports the situation with the command that resolves it, and `--adopt-backup-roots` hands the existing content over. Handing over is deliberately opt-in and never happens on boot: it is a one-off migration, not something to walk a multi-terabyte tree for on every start. The check itself stops at the first file it finds and gives up after ten seconds. Symlinks are re-owned without being followed, because their targets can point outside the backup root entirely.
