...
The bridge is source-only until an immutable image is published. Check out the exact `v1.1.5` tag and use the app-data-only Compose definition above. Do not import an old Community Applications template: those templates may restore the Docker socket, share mounts, peer port, or mutable `latest` image that this maintenance bridge deliberately removes. Earlier `v1.1.x` tags are superseded for production installation; `v1.1.5` includes the corrected auth boundary, large-database-safe workflow, packaged frontend, and actionable assessment guidance.
